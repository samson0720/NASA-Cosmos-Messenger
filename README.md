...
一個以對話形式瀏覽 NASA Astronomy Picture of the Day（APOD）的 Android App，使用者可以輸入日期查詢 APOD、收藏喜歡的天文圖片、離線回看快取內容、重新開啟後延續對話紀錄，並把 APOD 轉成看圖解說、生日星空卡或 3 張收藏圖片的回憶拼貼。
...
| Chat history persistence | Room / SQLite 保存 Nova 對話紀錄，重新開啟 App 後可還原 |
...
| Unit tests | 93 個 JVM unit tests，覆蓋核心規則、資料層、Room cache 與 chat history |
...
| Room | Favorites、APOD cache 與 chat history |
